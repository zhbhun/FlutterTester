...
const double _kMagnification = 1;
const double _kDatePickerPadSize = 12.0;
// The density of a date picker is different from a generic picker.
// Eyeballed from iOS.
// custom：squeeze 设置为 1.55
// const double _kSqueeze = 1.25;
const double _kSqueeze = 1.55;
...
class _DatePickerModalBody extends StatefulWidget {
  final String title;
  ...
  @override
  _DatePickerModalBodyState createState() {
    return _DatePickerModalBodyState();
  }
}

class _DatePickerModalBodyState extends State<_DatePickerModalBody> {
  DateTime _dateTime;

  
  @override
  void initState() {
    _dateTime = this.widget.initialDateTime;
  }

  _handleDateChanged(DateTime time) {
    setState(() {
      _dateTime = time;
    });
  }
  ...
  _cancel() {
    if (this.widget.onCancel != null) {
      this.widget.onCancel(this._dateTime);
    }
  }

  _confirm() {
    if (this.widget.onConfirm != null) {
      this.widget.onConfirm(this._dateTime);
    }
  }
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: CupertinoDatePicker(
              ...
              onDateTimeChanged: this._handleDateChanged,
            ),
          ),
        ],
      ),
...
class CupertinoDatePicker extends StatefulWidget {
  // custom: 显示日期弹框
  static showDateModal({
    @required BuildContext context,
    String title,
    DateTime initialDateTime,
    DateTime minimumDate,
    DateTime maximumDate,
    ValueChanged<DateTime> onCancel,
    ValueChanged<DateTime> onConfirm,
  }) {
    DateTime initial =
        initialDateTime == null ? DateTime.now() : initialDateTime;
    if (minimumDate != null && initial.isBefore(minimumDate)) {
      initial = minimumDate;
    } else if (maximumDate != null && initial.isAfter(maximumDate)) {
      initial = maximumDate;
    }
    showModalBottomSheet(
      context: context,
      builder: (context) {
        return _DatePickerModalBody(
          title: title,
          initialDateTime:
              initialDateTime == null ? DateTime.now() : initialDateTime,
...
class _CupertinoDatePickerDateTimeState extends State<CupertinoDatePicker> {
  ...
  Widget _buildMediumDatePicker(
      double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    return custom.CupertinoPicker.builder(
      scrollController:
          FixedExtentScrollController(initialItem: selectedDayFromInitial),
      // custom: 去掉 offAxisFraction
...
class _CupertinoDatePickerDateState extends State<CupertinoDatePicker> {
  ...
  Key dayKey = UniqueKey();

  Key monthKey = UniqueKey();
  ...
  Widget _buildDayPicker(
    ...
    List<Widget> widgets = List<Widget>.generate(31, (int index) {
      int day = index + 1;
      // print('>> day $day ${selectedYear} ${selectedMonth} ${selectedDay} ${widget.minimumDate} ${widget.maximumDate}');
    ...
    // print('>> day ${children.length}');
    int offset = 0;
    if (widget.minimumDate != null &&
        selectedYear == widget.minimumDate.year &&
        selectedMonth == widget.minimumDate.month) {
      offset = widget.minimumDate.day - 1;
    }
    ...
    return custom.CupertinoPicker(
      ...
      scrollController:
          FixedExtentScrollController(initialItem: selectedDay - 1 - offset),
      // custom: 去掉 offAxisFraction
      // offAxisFraction: offAxisFraction,
      itemExtent: _kItemExtent,
      useMagnifier: _kUseMagnifier,
  ...
  Widget _buildMonthPicker(
      double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    List<Widget> widgets = List<Widget>.generate(12, (int index) {
      int month = index + 1;
      // print('>> $month ${selectedYear} ${selectedMonth} ${widget.minimumDate} ${widget.maximumDate}');
      if (widget.minimumDate != null &&
          selectedYear <= widget.minimumDate.year &&
          month < widget.minimumDate.month) return null;

      if (widget.maximumDate != null &&
          selectedYear >= widget.maximumDate.year &&
          month > widget.maximumDate.month) return null;
    ...
    // print('>> month ${children.length}');
    int offset = 0;
    if (widget.minimumDate != null && selectedYear == widget.minimumDate.year) {
      offset = widget.minimumDate.month - 1;
    }

    return custom.CupertinoPicker(
      key: monthKey,
      scrollController: FixedExtentScrollController(
        initialItem: selectedMonth - 1 - offset,
      ),
      // custom: 去掉 offAxisFraction
      // offAxisFraction: offAxisFraction,
      itemExtent: _kItemExtent,
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          int newMonth = index + 1 + offset;
          if (widget.minimumDate != null &&
              selectedYear <= widget.minimumDate.year &&
              newMonth <= widget.minimumDate.month &&
            ...
            widget.onDateTimeChanged(
              ...
              selectedYear >= widget.maximumDate.year &&
              newMonth >= widget.maximumDate.month &&
            ...
            widget.onDateTimeChanged(
                DateTime(selectedYear, selectedMonth, selectedDay));
          } else if (newMonth != selectedMonth) {
    ...
    return custom.CupertinoPicker.builder(
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          int newYear = index;
          if (widget.minimumDate != null &&
              newYear <= widget.minimumDate.year &&
              (selectedMonth < widget.minimumDate.month ||
                  (selectedMonth <= widget.minimumDate.month &&
                      selectedDay < widget.minimumDate.day))) {
            selectedYear = widget.minimumDate.year;
            ...
            widget.onDateTimeChanged(
              ...
              newYear >= widget.maximumDate.year &&
              (selectedMonth > widget.maximumDate.month ||
                  (selectedMonth >= widget.maximumDate.month &&
                      selectedDay > widget.maximumDate.day))) {
            selectedYear = widget.maximumDate.year;
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        // custom: 支持日期限制
        if (widget.minimumDate != null && index < widget.minimumDate.year)
          return null;

        if (widget.maximumDate != null && index > widget.maximumDate.year)
          return null;
